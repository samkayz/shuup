...
from pytest_django.fixtures import django_username_field
...
from shuup.testing.factories import get_default_currency, create_product, get_random_filer_image, get_default_supplier
...
def test_permissions(admin_user, settings):
    ...
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert json.loads(response.content.decode("utf-8"))[0].startswith("Cannot load basket of a different Shop")
    ...
    print("asdf")
    client = _get_client(person_one.user)
    response = client.get("/api/shuup/basket/{}-{}/".format(shop_one.pk, basket.key))
    assert response.status_code == status.HTTP_200_OK
    ...
    assert basket.shop == shop_one
    assert basket.creator == admin_user
    # ok, person two is not allowed either
    client = _get_client(person_two.user)
    response = client.get("/api/shuup/basket/{}-{}/".format(shop_one.pk, basket.key))
    ...
    # but admin is, yay admin!
    client = _get_client(person_three.user)
    response = client.get("/api/shuup/basket/{}-{}/".format(shop_one.pk, basket.key))
    assert response.status_code == status.HTTP_200_OK
